/**
 * FluidCursor — global WebGL fluid-simulation cursor effect.
 *
 * Renders a fixed, full-viewport, click-through (`pointer-events: none`) canvas
 * that paints a colorful fluid trail following the pointer, with a burst on click.
 *
 * Notes
 * - The whole simulation is scoped inside `runFluidCursor(canvas)` (no module
 *   globals) so it tears down cleanly on unmount and survives React StrictMode's
 *   double-mount in dev.
 * - Disabled when the user prefers reduced motion.
 * - On WebGL failure it bails silently (no DOM takeover) so the app keeps working.
 */
...
export default function FluidCursor() {
  ...
  useEffect(() => {
    ...
    let cleanup = () => {};
    try {
      cleanup = runFluidCursor(canvas) || cleanup;
    } catch (error) {
...
/**
 * Runs the fluid simulation against the given canvas.
 * Returns a cleanup function that stops the loop and removes listeners.
 */
function runFluidCursor(canvas) {
  const config = {
    SIM_RESOLUTION: 128,
    DYE_RESOLUTION: 1440,
  ...
  // Scoped state (was module-global in the original script)
  let gl, ext;
  ...
  function generateColor() {
    // Brighter than the original 0.15 so the smoke reads as vivid color
    // over a light background instead of a dark/blackish smudge.
  ...
  function render(target) {
    // Clear the visible canvas to fully transparent each frame so only the
    // smoke is composited over the page (no opaque/black backdrop).
    if (!target) {
      gl.bindFramebuffer(gl.FRAMEBUFFER, null);
      gl.viewport(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight);
  ...
  // Event listeners (tracked so we can remove them on cleanup)
  ...
  // Cleanup — stop the loop and detach listeners. We intentionally do NOT
  // force-lose the WebGL context here: under React StrictMode the effect is
  // cleaned up and re-run against the same <canvas>, and a lost context would
  // brick the re-init (black/non-rendering canvas).
  return () => {
    destroyed = true;
